...
\begin{example}
  ...
  pi and p in sigma} shows us two things. On the left side, we see that $p = 
...
It is easy to see that the cover indeed generates the permutations that avoid 
...
discarding symmetries. That made for 163 additional avoidance sets to find covers for, 
...
\begin{figure}[htbp]
  ...
  \begin{tabular}{ r c l l }
    ...
    \strule{\pattdispscale}{3}{3}{
      ...
      (1,2)/\point{2pt},
      (2,1)/$\mc{C}$
    }$ & $\mc{F} = \Av{ \mpattern{scale=\patttextscale}{ 2 }{ 1/1, 2/2 }{ 0/0, 0/1, 0/2, 1/0, 1/1, 1/2, 2/0, 2/2 } }$
  \end{tabular}
...
those that avoid the mesh pattern $p = \mpattern{scale=\patttextscale}{ 3 }{ 
